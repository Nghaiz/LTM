...
try {
    ...
    Write-Host ""
    Write-Host "  Throwable regression pass (repeat for FRAG, SPEARHEAD, AMMO BAG, MEDIPACK):"
    Write-Host "     a. Throw every available use. Each click must create exactly one visible object;"
    Write-Host "        loaded + reserve must fall by exactly one only after the release moment."
    Write-Host "     b. Hold fire through the animation and press reload. Neither action may duplicate"
    Write-Host "        the held model, projectile, or restore a spent use."
    Write-Host "     c. Begin a throw and switch weapon before release; repeat while being killed."
    Write-Host "        Both cancellations must create no projectile and must preserve that use."
    Write-Host "     d. Watch the same actions from another client. Frag and spearhead must retain"
    Write-Host "        different models, and ammo/med bags must never render as grenades."
    Write-Host ""
