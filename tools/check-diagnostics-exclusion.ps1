...
# carried forward, and it happened on 2026-08-26: phase C4d landed the asmdef, RULE 1 and RULE 2
# were replaced by its defineConstraints line, and RULE 3 by the compiler. RULE 4 outlived all
# three and is what this file now is. See the block where they used to be.
#
...
# ---------------------------------------------------------------------------------------------
# RULES 1, 2 AND 3 WERE DELETED BY PHASE C4d, on this file's own instruction (see the header).
#
# RULE 1 required every file in the folder to open with `#if !IRONFRONT_NO_DIAGNOSTICS` and
# RULE 2 required the guard to still be open at EOF. Both are now enforced by one line in
# Assets/Scripts/Net/Diagnostics/Ironfront.Net.Unity.Diagnostics.asmdef:
#
#     "defineConstraints": [ "!IRONFRONT_NO_DIAGNOSTICS" ]
#
# With that present the WHOLE ASSEMBLY is excluded from a build carrying the define, so a file
# that forgot its guard is excluded anyway and a guard that closed early protects nothing that
# was at risk. Enforcing a per-file convention that no longer decides the outcome is how a gate
# becomes ceremony.
#
# RULE 3 required that nothing outside the folder names a type declared inside it, so that a
# strip could not leave a dangling reference. That is now the compiler's job and it is strictly
# better at it: Net/Diagnostics is its own assembly, so a caller outside it must hold an explicit
# assembly reference, and a build that excludes the assembly fails to compile any file that
# names one of its types. A source-text scan approximated this; an assembly boundary IS it.
#
# THE `#if` GUARDS THEMSELVES STAY IN THE FILES, deliberately, and are no longer checked. They
# cost nothing, and they are the fallback if the defineConstraints line is ever dropped — which
# is exactly the regression check-net-layering.ps1 RULE 7a now watches for. Belt and braces,
# with the braces gated and the belt not.
#
# RULE 4 outlives all three, exactly as the header predicted: nothing about an asmdef proves that
# the excluded configuration still BUILDS, and a strip nobody exercises is a strip nobody knows
# works.
...
Write-Host "PASS: RULE 4 — EditorBuildWindowsHarness still builds the excluded configuration,"
Write-Host "      so the strip is demonstrated rather than asserted."
Write-Host ""
Write-Host "      THIS RUN CHECKED ONE THING. Rules 1-3 were deleted by phase C4d and are NOT"
Write-Host "      silently passing: the per-file guard convention is now enforced by the"
Write-Host "      defineConstraints line on Ironfront.Net.Unity.Diagnostics.asmdef, and the"
Write-Host "      dangling-reference scan by the assembly boundary itself. That the asmdef still"
Write-Host "      carries that line is checked by check-net-layering.ps1 RULE 7a, not here."
exit 0
