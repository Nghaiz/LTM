...
public class Actor : Hurtable
{
	...
	protected virtual void FixedUpdate()
	{
		...
		if (inWater)
		{
			...
			hipRigidbody.linearDamping = 8f;
			hipRigidbody.angularDamping = 8f;
			if (!dead)
			{
				headRigidbody.AddForce(-Physics.gravity * 3.5f, ForceMode.Acceleration);
				headRigidbody.angularDamping = 3f;
				headRigidbody.linearDamping = 10f;
			}
		}
		else
		{
			hipRigidbody.linearDamping = 0f;
			headRigidbody.linearDamping = 0f;
			hipRigidbody.angularDamping = 0.05f;
			headRigidbody.angularDamping = 0.05f;
		}
