...
namespace Ironfront.Net.Unity
{
    ...
    public static class DisplayResolutionCatalog
    {
        public static DisplayResolutionOption[] Build(IEnumerable<DisplayResolutionOption> source)
        {
            ...
            var unique = new List<DisplayResolutionOption>();
            foreach (DisplayResolutionOption option in source)
            {
                if (option.Width <= 0 || option.Height <= 0) continue;
                if (seen.Add((option.Width, option.Height))) unique.Add(option);
            }

            return unique.OrderBy(option => (long)option.Width * option.Height)
                .ThenBy(option => option.Width).ToArray();
        }

        public static int FindBestIndex(IReadOnlyList<DisplayResolutionOption> options,
            int width, int height)
        {
            ...
            for (int index = 0; index < options.Count; index++)
            {
                ...
                if (option.Width == width && option.Height == height) return index;
