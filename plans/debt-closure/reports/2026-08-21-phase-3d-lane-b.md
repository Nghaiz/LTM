...
| 2 | **`NetLog` has no sink in the shipped player.** Every transport warning a production client or dedicated server raises is discarded. | § 4 |
| 3 | **`AiActorController.Die` NREs on the headless server** — `squad.DropMember(this)` with a null `squad` (`AiActorController.cs:1911`), reached from `Actor.Damage`. 676 occurrences in one 90-second run, repeating because the NRE aborts the rest of `Actor.Die` and the bot dies again next frame. | `artifacts/lane-b/combat-01/server.log` |
| 4 | **Respawn cannot be scripted, or rebound.** `NetClientLocalCombatDriver` reads `Input.GetKeyDown(_respawnKey)` directly; there is no `IInputSource` path to `C_SPAWN_REQUEST`, so no controller, no rebind and no scripted client can respawn. Check 13's death and death-screen halves are reachable; the respawn press is not. | `NetClientLocalCombatDriver.cs:123` |
| 5 | **Grenade parity (check 4) is not scriptable.** `InputButtonPacker` declares no grenade bit, because nothing in `FpsActorController` produces one. Carried from the 3D opening commit; restated here because it is a check that cannot be run rather than one that failed. | `InputButtonPacker` |
