...
| [phase-02](phases/phase-02-combat.md) | 7–10 | M2 | Prediction glue + reconciliation · shooting/health/death/respawn · local ragdolls · **+ front-loaded UI** · **+ 1 backup week for Dev C** |
...
>    itemized
> 2. You absorb every request from Dev C ("expose this extra field", "change that in `Actor.cs`")
> 3. You carry all the visual debugging — when a remote player looks wrong, you're the only one who
>    can fix it

### 4.2. You are Dev C's backup

Dev C is the highest-risk role on the team (47/70 difficulty, 3 dependencies). If C is away, the
...
> `Actor.cs` **moved to Dev C**. That was your riskiest refactor under the old plan (phase-02,
> week 7), and it was also the worst blocker in the whole project. See
> [dependency-map.md § 4](../00-shared/dependency-map.md).
>
> What's left for you: Dev C hands you a finished `MovementSimulation.cs`; you just **call it** from
