...
| A13 | **An authored comment in our own porting work is wrong, and that is worse than the code.** `MinimapUi.cs:100-102` states bots use `RandomFrontlineSpawnPointForTeam`; `AiActorController.SelectedSpawnPoint` (`:2109`) actually takes a **30% branch to `RandomSpawnPointForTeam`** first. Porting from the comment drops rear-spawning silently, and grepping `SelectedSpawnPoint` never surfaces the other method. **Owner: V8.** Fix the comment in the same commit as the behaviour | Verified at source, 2026-08-17 |
