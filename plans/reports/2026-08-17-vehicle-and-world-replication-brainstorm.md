...
| A12 | **`MinimapUi` spawn-point selection is hardcoded to team 0.** `:129` sets `int num = 0`, never reassigns it, and `:140` gates on `owner == num` — so no team-1 player can select a spawn point. Invisible in single-player, where the human is always team 0. Compounded by a bare `catch (Exception) { }` at `CapturePoint.cs:262-268` that has been swallowing it, and by a null guard at `:125` that checks `instance` while `:130` dereferences a dictionary built later in `SetupMinimap()`. **Owner: V10** (client-side UI reading local team); V8 preserves the call path untouched, so V8 must cross-reference it | Verified at source, 2026-08-17 |
| A13 | **An authored comment in our own porting work is wrong, and that is worse than the code.** `MinimapUi.cs:100-102` states bots use `RandomFrontlineSpawnPointForTeam`. Two things it omits: `AiActorController.SelectedSpawnPoint` (`:2109`) takes a **30% branch to `RandomSpawnPointForTeam`** first, *and* `RandomFrontlineSpawnPointForTeam` itself falls back to `RandomSpawnPointForTeam` at `ActorManager.cs:216` when no owned point satisfies `!IsSafe() \|\| IsFrontLine()`. So `RandomSpawnPointForTeam` is the **universal floor**, not a 30% alternative — bots reach it explicitly and both bots and players reach it implicitly. Reimplementing that two-level chain twice server-side will drift bot and player spawns apart in a way the original never did. Grepping `SelectedSpawnPoint` surfaces none of it. **Owner: V8.** Fix the comment in the same commit as the behaviour | Verified at source, 2026-08-17 |
| A15 | **Both turrets hijack the local player's cameras, and it arms only once V10 lands.** `TankTurret.Unholster()` (`:41-45`) and `MountedTurret.Unholster()` (`:31-35`) both call `FpsActorController.instance.DisableCameras()` gated solely on `!user.aiControlled`. This cannot fire today because remote actors are bare `Transform`s with no controller — the moment V10 gives them an `Actor`, a **remote** player entering a turret blacks out the **local** player's rig. The guard is on `Unholster`, not a combat event, so a V10 review scoped to combat events never reaches it. This is the concrete, severe form of "`aiControlled` has no correct value for a remote actor" (`Actor.cs:178`), and it interacts with V5-D7. **Owner: V10** | Verified at source, 2026-08-17 |
| A16 | **A lossy stream cannot drive a stateful counter.** `S_WEAPON_FIRE` is documented safe-to-drop (`CombatMessages.cs:139-141`), but `AlternatingMountedWeapon.MuzzlePosition()` (`:19-22`) reads `currentMuzzle`, advanced once per shot at `:12`. Any cosmetic seam that advances the counter from received fire events desynchronises on the first dropped packet. The muzzle seam must be **stateless** — the muzzle index rides the event, or the cosmetic does not depend on it. **Owner: V10**, with V6 replicating `currentMuzzle` for the authoritative path | Verified at source, 2026-08-17 |
| A14 | **The duplicated spawn-ownership check is load-bearing, not redundant.** `MinimapUi.cs:115-118` detects an enemy-owned point, calls `LoadoutUi.Show()`, then **returns the enemy point anyway**; `FpsActorController.cs:682-685` is what actually blocks it by returning null. Deleting the apparently-redundant check would let players spawn on enemy points. `ActorManager.cs:137` `actor.controller.SelectedSpawnPoint()` is the single polymorphic interception point for server-side spawn selection, and it sits inside a coroutine. **Owner: V8** | Verified at source, 2026-08-17 |
