# Plan — Dev C · Replication & Server Simulation
...
> exchange for Dev B and Dev D having **zero dependencies** after week 2.
...
4. **`MovementSimulation` — the shared truth of client and server.** *(newly taken from Dev A)*
...
| `tools/run-integration.ps1` + integration scenarios | **Owner — newly taken from Dev B** |
...
### 2.1. You verify, Dev B implements

| | Who does it |
|---|---|
| Implementing bit-packing + quantization | **Dev B** |
...
- If Unity generates new `.meta` files → **don't commit them**, leave it to Dev A
...
|---|---|---|---|
| [phase-00](phases/phase-00-foundation.md) | 1–2 | M0 | Chair the protocol freeze · `ProtocolConstants` · **the conformance test suite** (the referee) · **start extracting `MovementSimulation`** · `InputSerializer` |
| [phase-01](phases/phase-01-snapshot.md) | 3–6 | M1 | Full + delta snapshots · `ServerTickLoop` · authoritative input application · 2 clients in sync |
| [phase-02](phases/phase-02-lag-comp.md) | 7–10 | M2 | Interest management · hitbox history · lag compensation · server-authoritative shooting · bot replication |
...
| [phase-04](phases/phase-04-report.md) | 14 | M4 | Benchmarks · the data-compression report · documentation |
...
| **`MovementSimulation` — extracting it from `Actor.cs`** | **1.5** | **+1.5 taken from Dev A** |
...
| **Integration harness + benchmarks** | **1.5** | **+0.5 taken from Dev B** |
...
|---|---|---|---|
| 1 | Extracting `MovementSimulation` from `Actor.cs` | Dev A → **you** | The worst blocker in the old plan (week 7, mid-project, on the hardest piece). The file must be identical on client and server; you're the one who suffers when it diverges |
| 2 | The bit-packing serializer | **you** → Dev B | Isolated byte-level work, squarely B's strength, and it keeps B dependency-free. You keep the referee role (conformance tests) |
| 3 | The integration harness | Dev B → **you** | You sit in the middle, so you should be the one running and fixing integration when it breaks |
