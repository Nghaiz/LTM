# Dev C — Phase V3: Protocol v3.0.0 — the vehicle wire
...
design of record § 9 already scores `Actor.cs`/Dev A branch conflict at 12 for exactly this reason.
...
### Task 7 — `VehicleIds` registry and the asset side (S, 1 day + a Dev A review round)
...
`Vehicle.cs` is a Dev A file. Per the design of record § 7 the code is written here, with a PR and
a Dev A review round; **Dev A authors the per-prefab values**, the same way the per-weapon
...
```
| **3.0.0** | Week 3 | Dev C | **The vehicle wire.** Six new opcodes (0x21, 0x4C–0x50) and
...
| `Vehicle.cs` conflicts with V0's edits or Dev A's branch | 3 | 3 | 9 | V3 depends on V0 (§ 6 of the design of record), so V0's `Vehicle.cs` edits land first and Task 7 rides on top. Task 7 is one serialized field and is severable — it can land in its own PR after the protocol PR. |
...
| 7 — `VehicleIds` + `Vehicle.networkId` | S (1d) + Dev A review | Needs V0 merged. Severable; can land in its own PR. |
...
be split into a second PR if the Dev A round runs long.
...
**To Dev A.** One field: `[SerializeField] private byte networkId` on `Vehicle`, plus authoring the
per-prefab values against `protocol-spec.md § 4.9`. Same shape as the per-weapon `Configuration`
values already owned there. Nothing else in this phase touches a Dev A file, and nothing in it needs
the Editor.
