# Dev C — Phase V2: Weapon configs, or the seventeen guns that are all the same gun
...
> (ownership). Per the design doc § 7, Dev C writes every file in this phase; Dev A owns only the
> Editor half.
>
...
so: *"until a loadout message or Dev A's weapon assets say otherwise"*. Nothing ever said otherwise.
...
| **D2** | **Placeholder values ship, and shipping them is the deliverable.** The real per-weapon numbers exist only as serialized fields inside `Ironfront_Reborn/Assets/Resources/_Managers.prefab`, which the server cannot read — `WeaponIds.cs`'s own doc block says exactly this, and calls a field whose meaning lives in a file only one side can open "the same defect the channel envelope had for a whole milestone". V2 ships the **shape** with placeholders derived from weapon class; Dev A fills the numbers. Because the seam takes a `WeaponConfig`, swapping numbers is data, not code. This is the design doc § 9 risk row, executed rather than deferred. |
...
| `EveryCatalogEntryIsMarkedUnauthored` | D3 — `AuthoredCount == 0` today. **This test is meant to fail when Dev A fills the numbers**, which is exactly the point: the phase that supplies real values must consciously flip the flags. |
...
**To Dev A** — one thing, and it is the deliverable this phase is designed around:
...
is a playtest question and depends on Dev A's handoff above being done.

**Still outside Dev C:** the prefab values themselves (Dev A), and nothing else.
