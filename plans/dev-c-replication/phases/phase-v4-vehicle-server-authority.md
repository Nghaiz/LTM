# Dev C — Phase V4: Vehicle server authority
...
> track edit `Assembly-CSharp/` files by PR plus a Dev A review round).
>
...
7. All of it is graded by tests that run in CI under `dotnet test` with no Editor. The checks that
   genuinely need Unity are named and handed to Dev A.
...
| **V4-D6** | **Seat identity on the wire is `(vehicleId u16, seatIndex u8)`.** `Seat.cs` gains no id field. The index is the index into `Vehicle.seats`, which is already the handle `Actor.SwitchSeat` uses (`Actor.cs:1064`). `seats[0]` stays the driver **by array-index convention** — `Seat.Type.Driver` remains unconsulted, exactly as today (`Vehicle.cs:118`, `:123`, `:190`, `:224`). Changing that convention is prefab data, i.e. Dev A. V4 **pins** it with a test instead of changing it. |
...
| `Ironfront_Reborn/Assets/Scripts/Assembly-CSharp/Vehicle.cs` | **Edit** (Dev A file — PR + review round, design § 7). A role guard at the top of `Damage`, the same shape as phase-05 Task 6's guard on `Actor.Damage`: **server** routes the health change through the sink then lets the rest run; **client** runs the cosmetics (`damageParticles`, `impactAudio`, `heavyDamageAudio`) but skips the `health -=` and the `StartBurning()` / `Die()` — those arrive from the snapshot flag and `S_VEHICLE_DESPAWN`; **offline** is an early no-op and behaves exactly as today. | Unity |
...
| `Ironfront_Reborn/Assets/Scripts/Assembly-CSharp/Vehicle.cs` | **Edit** (Dev A file, same PR as Task 5). `seatsClaimedByBots` becomes computed at server role from `BotSeatClaims`; `ClaimSeat` / `DropSeatClaim` / `HasUnclaimedSeats` route to it at server role and are unchanged offline. No stored duplicate — `code-conventions.md` § "No Derived Fields". | Unity |
...
Dev C-owned files only (`Net/Server/**` is C's per conventions § 7), except the two
...
Profiler run that proves it is **Dev A's**, out of scope here — same boundary phase-05 Task 3 drew.
...
| `Vehicle.cs` role guards | **Unity** (Dev A file) | The damage choke point lives there |
...
| Dev A's review round on the two `Vehicle.cs` edits slips the phase | 3 | 2 | 6 | Tasks 5 and 6 batch their `Vehicle.cs` edits into **one** PR, and everything else merges without it — the same severability phase-05 D8 used. |
...
| `Vehicle.cs` conflicts with Dev A's branch | 3 | 3 | 9 | One PR, one file, announced before it opens — the design § 9 mitigation applied one file over from `Actor.cs`. |
...
**To Dev A.** One PR against `Assembly-CSharp/Vehicle.cs` carrying both the `Damage` role guard
