# Dev C — Phase V8: Objectives — one capture authority, and a spawner that survives a headless server
...
session — but this phase produces the single largest Dev A item in the track (§ 7), and that item
...
| **D9** | **Elimination moves into `MatchStateMachine`; scoring does not.** `ScoreUi` holds match state in a UI component and its own doc comment already flags it (`ScoreUi.cs:46-57`): on a headless server the original game neither scores nor ends. Elimination is a *loss condition* and belongs to the authoritative match. Score, `ScoreMultiplier(flags)` (`:142-145`) and the `victoryPoints` race (`:77-84`) are a **rendering-and-rules redesign** that is Dev A's call, and the networked match uses tickets instead. Recorded as a deliberate divergence, not a defect to fix here. |
...
**Files:** `Ironfront_Reborn/Assets/Scripts/Assembly-CSharp/CapturePoint.cs` (Dev A file — PR plus
...
**Files:** `Ironfront_Reborn/Assets/Scripts/Net/Server/MatchController.cs` (Dev C).
...
**Files:** new `Ironfront_Reborn/Assets/Scripts/Net/Server/CapturePointSlave.cs` (Dev C);
`MatchController.cs` (Dev C).
...
`Ironfront.Net.Replication/Match/MatchRules.cs` (Dev C); `MatchController.cs` (Dev C).
...
**Files:** new `Ironfront.Net.Replication/World/VehicleSpawnScheduler.cs` (Dev C); new
`Ironfront_Reborn/Assets/Scripts/Net/Server/NetVehicleSpawner.cs` (Dev C);
`Assembly-CSharp/VehicleSpawner.cs`, `Assembly-CSharp/Vehicle.cs` (Dev A files — one PR, one review
round).
...
| `CapturePoint.cs` / `Vehicle.cs` / `VehicleSpawner.cs` conflict with Dev A's branch | 4 | 3 | **12** | All three land in **one** PR, early, announced. Same precedent as phase-05 Task 6. Tasks 2-4 touch only Dev C files and merge independently of that review round. |
...
| Task | Effort | Notes |
|---|---|---|
| 1 — Split `CapturePoint` | M (1.5d) | Dev A file. Starts the review round early, so it does not gate the rest. |
| 2 — Bind `MatchController` to components | S (1d) | Dev C only. Needs Task 1's `CaptureSpeed` field. |
...
| 5 — `VehicleSpawner` authority | M (1.5d) | Dev A files; rides Task 1's PR. Independent of 1-4. |
...
**To Dev A — one PR, three files, and one Editor task that blocks V9.**
...
Also Dev A's, unchanged from design § 7: `.meta` files for `CapturePointSlave.cs`,
...
**Not in this phase and not Dev A's either:** the `ScoreUi` redesign (D9). It is recorded in the
