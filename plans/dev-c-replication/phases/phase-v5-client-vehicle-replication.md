# Dev C — Phase V5: Client vehicle replication
...
> ownership — with the recorded departure in design § 7 for `Assembly-CSharp/` and Dev A's
...
5. Graded in CI without the Editor wherever the quantity is arithmetic; the checks that genuinely
   need Unity are named and handed to Dev A.
...
| `Ironfront_Reborn/Assets/Scripts/Assembly-CSharp/Vehicle.cs` and the four subtypes | **Edit** (Dev A files, batched into V4's existing PR where possible). A `NetContext.IsClient` guard that disables the drive path on a non-predicted vehicle, and drives `Helicopter.rotorSpeed` / `Car.steerAngle` from the replicated subtype tail instead of from local integration. Offline is an early no-op and behaves exactly as today. | Unity |
...
surviving local force would fail it). Rendering itself is Dev A's — see § 4.
...
### Task 5 — The input seam: `IInputSource` vehicle axes and `C_VEHICLE_INPUT` (2.5 days + a Dev A review round)
...
| File | Change | Side |
|---|---|---|
| `Ironfront_Reborn/Assets/Scripts/Net/Input/IInputSource.cs` | **Edit** (Dev A file — PR + review). Four members: `HeliYaw`, `HeliCollective`, `HeliRoll`, `HeliPitch`. No `using UnityEngine` — the file and its pure siblings are `<Compile Include>` linked into `Ironfront.Client.Input.Tests`, and adding one silently drops them out of coverage (`IInputSource.cs:13-20`). | Unity-adjacent, engine-free by construction |
...
### What genuinely needs the Editor — handed to Dev A
...
| `FpsActorController`, `Vehicle` + subtypes | **Unity** (Dev A files) | The pull sites and the drive paths live there |
...
| A cosmetic that read local physics breaks when remote vehicles go kinematic | 4 | 3 | **12** | Task 3 enumerates the six sites and drives the two that matter from the replicated subtype tail — which is why design § 5 reserved it. The remaining risk is one nobody listed; that is explicitly Dev A's Editor check in § 4. |
...
| The `IInputSource` edit is a Dev A file and the review round slips the phase | 3 | 2 | 6 | Task 5 is severable and last (phase-05 D8 precedent). Tasks 1–4 merge without it; remote interpolation — the half two of three clients see — does not depend on it. |
...
| 5 — Input seam + `C_VEHICLE_INPUT` | M (2.5d) + review round | Severable, last. Needs a Dev A round on `IInputSource` and `FpsActorController`. |
...
**To Dev A.** One PR against `Net/Input/IInputSource.cs`, `LocalInputSource.cs`,
