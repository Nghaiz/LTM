# Dev C — Phase V9: Integration and measurement — grading the thirteen criteria
...
7. Each of the design's thirteen criteria carries a verdict, an artefact, and — where it genuinely
   needs the Editor — a named Dev A owner rather than a blank.
...
**Files:** new project `Ironfront.Net.LoadHarness/` (Dev C); new
`Ironfront_Reborn/Assets/Scripts/Net/Server/HeadlessLoadBootstrap.cs` (Dev C).
...
**Files:** `Ironfront.Net.Replication/Server/ServerStateAudit.cs` (Dev C).
...
**Files:** new `plans/dev-c-replication/reports/phase-v9-report.md` (Dev C).
...
**What genuinely needs the Unity Editor**, and therefore goes to Dev A rather than being marked
failed (design § 7):
...
| 10 | The allocation half | Tick p99 is measured here. The Profiler run that proves the tick loop allocates nothing per frame is the same S4 evidence outstanding since M1, and it is Dev A's. |

Everything else — 3 through 9, 11, 12, 13 — is graded in this phase, in CI or on a headless run,
with no Editor session.

Also handed to Dev A, as prerequisites rather than results: the `MatchController._capturePoints`
...
| Criterion 2 is perceptual and has no owner, so it stays blank | 3 | 2 | 6 | Task 7 assigns it to Dev A's two-client Editor test explicitly, with the harness's correction-magnitude and correction-frequency numbers attached as supporting evidence. |
...
**To Dev A — three items, one of them blocking.**
...
Also Dev A's, unchanged: per-weapon `Configuration` values in `_Managers.prefab` (V2 ships the table
...
**To Dev D:** the harness's 16-client connect-storm and abrupt-disconnect programmes are the same
