# Adversarial Review — PR #28 (the master-server track, Master Server Phase 00 Foundation)

Reviewed at `d:/Coding/LTM-pr28`, branch `feat/dev-d-phase-00-foundation`.
Read-only review. Scope gated to the master-server track's own commits (`a94bd87`, `f8d00d2`) — the other ~220 files
in the branch-vs-`main` diff come from merging `develop` and are not the master-server track's work.
...
- two or more frames in one segment — i.e. the `while (true)` drain loop at `ClientConnection.cs:209`,
  which the master-server track's own comment calls "phase-00 trap 1";
...
suite `Ironfront.Net.Protocol.Tests/Conformance/MspFramingTests.cs` (the replication track's) — glued triples, one
message across five sends, one byte at a time, over-64 KB faults, max-length accepted, faulted-latch
sticky, sub-msgType length rejected. So the parser is not the risk; the risk is confined to the master-server track's
...
(the host binds `AddressFamily.InterNetwork`, per `TcpListenerHostOptions.cs:18`), and the master-server track
...
1. **SSOT: clean.** `ClientConnection` uses the shared `MspFrameReader` / `MspFrame` /
   ...
   `git show --stat a94bd87` confirms the master-server track did not touch `Ironfront.Net.Protocol` at all.
2. **Ownership: clean** (conventions §7). The commit touches only `Ironfront.MasterServer/**` (D),
   ...
   `plans/master-server/**` (D). Nothing in Protocol, Transport, Replication, or
