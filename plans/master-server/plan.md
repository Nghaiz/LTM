# Plan — the master-server track · Master Server & Services
...
| `phase-01` | 3–6 | M1 | Auth + SQLite · sessions · room registry · `IMasterClient` · **the `LoadTest` harness** |
| `phase-02` | 7–10 | M2 | Matchmaking · HMAC joinTickets · game server registry + heartbeat · chat |
| [phase-03](phases/phase-03-operations.md) | 11–13 | M3 | VPS deployment · monitoring · a 16-client load test · durability |
| `phase-04` | 14 | M4 | The TCP report · operations documentation |
...
**Done, 2026-08-15, by the lead rather than by the master-server track** — plus a fifth gate the original list did
...
The client track's desk.

**Not done, and not doable by the master-server track:** branch protection on `main`/`develop`. It needs repo
admin, which no collaborator has, on a plan that does not include the feature for private
repositories. Only  can resolve it — three options in
`docs/branch-protection.md` § Status. `.github/CODEOWNERS`
