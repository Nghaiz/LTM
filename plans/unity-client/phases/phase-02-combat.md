# the client track — Phase 02: Combat and prediction
...
> - **Lost:** extracting `MovementSimulation` (−1.5 pw) → moved to the replication track
> - **Gained:** front-loading UI from phase-03 (+1.5 pw) → eliminates the W11–13 crunch
> - **Gained:** 1 week backing up the replication track (+1.0 pw) → insurance for the team's riskiest role
>
...
>
> Extracting the movement logic out of `Actor.cs` **moved to the replication track**. Reason: this file has to be
...
**What you receive from the replication track:** a finished `Assets/Scripts/Net/Shared/MovementSimulation.cs`,
verified by C to match the original behavior. Due: **start of week 7**.

**What you provide to the replication track** (due week 2 — C needs it to start the extraction):
...
```csharp
// Owned by the replication track — you only call it
...
Build the UI shell **against fake data**, without waiting on the master-server track or the replication track. Weeks 11–13 then only
...
### Task 8 — Back up the replication track (1 week, spread across W7–10)

The replication track is the team's highest-risk role (47/70 difficulty, 3 dependencies, blocks you). If C is away
...
| Risk | Sign | Handling |
|---|---|---|
| Prediction constantly diverges from the server | Reconciling every tick, steady stutter | The client and server are running different logic. **Tell the replication track immediately** — C owns `MovementSimulation`. Give C: position logs from both sides at the same tick |
...
| The replication track delivers `MovementSimulation` after the week-7 deadline | Nothing to call | Write a minimal temporary version (walking + gravity only) using the constants C published, marked `// TEMPORARY — replace when C delivers`. Don't wait |
