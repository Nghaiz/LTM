# Plan — the transport track · Transport Layer (hand-written UDP)
...
### 2.1. You implement, the replication track verifies
...
| Conformance tests with hand-written hex | **The replication track** | `Ironfront.Net.Protocol.Tests/Conformance/` |
...
|---|---|---|---|
| `phase-00` | 1–2 | M0 | Socket refresher · project setup · raw send/receive in `UdpPeer` · **`NetworkSimulator`** · echo test · **`BitWriter`/`BitReader`** (`PacketHeader` and `Quantize` already shipped — see § 2) |
| `phase-01` | 3–6 | M1 | Handshake · seq/ack/bitfield · retransmit · 4 channels · fragmentation · RTT · ≥40 unit tests |
| `phase-02` | 7–10 | M2 | Congestion control · flow control · DoS defenses · 16 simultaneous connections · benchmarks |
...
| `phase-04` | 14 | M4 | Measurement report comparing against TCP · documentation · defense |
...
>
> 1. **Took the bit-packing serializer from the replication track** (+2.0). It's byte-level work, fully isolated,
>    testable with xUnit — squarely in your wheelhouse, and it keeps you at **zero dependencies**.
>    **The replication track writes the conformance tests that verify your code** (see § 4.1) — you implement, C
>    referees.
> 2. **Handed back the "integration support" burden** (−1.0) and **advanced flow control** (−0.5) to
>    the replication track, who owns the integration harness. You keep only 0.5 weeks for answering questions.
>
