...
Branch `feat/transport-phases-0-2` · worktree `d:/Coding/LTM-pr25` · 2,446 lines reviewed
...
1. **Interop break with the referee.** the replication track's conformance hex-sample suite (§ 14, *"the referee
...
3. `ConnectAcceptedPayload(connectionId, 0, 0, 0)` (`UdpTransportServer.cs:358`) hardcodes
   ...
   host to supply them. Spec § 3.1 requires all four fields. The client track/D cannot learn their `playerId`
...
The client track/C/D build on it — it is the one finding that cannot be fixed inside this project alone.
