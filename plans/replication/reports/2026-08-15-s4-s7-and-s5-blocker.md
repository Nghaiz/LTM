# the client track — S4 evidence, S5 blocker, and S7 wiring
...
`AiActorController.enabled`, is not accepted by the client track without a wrapper change. The
...
the eight coroutine workloads. The replication track needs to provide or agree on a Unity integration seam
that gates all intended AI work and exposes a deterministic LOD-off override. After that
lands, the client track can record the requested 32-bot p99 and AI before/after numbers.
