...
- **Week:** 2 / 14 *(phase 01 is scheduled for weeks 3–6; the engine-free half was pulled forward
  because it does not need the Editor and the client track was the critical path)*
- **Phase:** `phases/phase-01-snapshot.md`
...
encoding against acked baselines, and an end-to-end integration test that runs a server, the transport track's
...
with the −200 / +200 split declared in `[DefaultExecutionOrder]` so the client track's project settings are
untouched. What is left is not code: **criteria 1 and 9 need somebody to press Play and read the
Profiler** (checklist S4), and **criterion 7 is blocked on the transport track, not on the Editor** —
...
| 7 | 2 Unity clients see each other in sync | ☐ **blocked on the transport track** | `LoopbackTransport` is in-process — one Editor, one client, never a second process. Two clients needs the UDP transport. The offline equivalent passes: `SnapshotFlowIntegrationTests` runs server + loopback + fake client at lan/typical/bad and converges exactly |
...
**6 met, 3 met in code and awaiting an Editor run, 1 blocked on the transport track.**
...
| Bit packing (`BitStreamTests`, verifies the transport track) | 25 | 25 | 0 |
...
| C-01-11 | Trap 1 wants two execution orders, which the phase document assigns to `ProjectSettings` | `[DefaultExecutionOrder]` on the three components | Ask the client track to edit `ProjectSettings/ScriptExecutionOrder` | A cross-owner dependency for a value that never changes. The attribute is visible in a diff, travels with the file, and cannot be silently absent |
...
|---|---|---|
| Criteria 1 & 9 under real load — tick time p99 and 0 alloc/tick with Unity physics + AI | the client track — Editor only | ☑ checklist **S1–S4**. The Unity loop now exists; what is left is pressing Play and reading the Profiler |
| Criterion 7 — two Unity clients in sync | **The transport track — UDP transport** | `LoopbackTransport` is in-process and reaches exactly one client. Not an Editor problem and not the client track's to unblock |
| ~~`ServerTickLoop` script execution order (trap 1)~~ | ~~the client track owns project settings~~ | ✅ **Closed without asking.** Declared in `[DefaultExecutionOrder]` on `NetServerBootstrap` (−1000), `ServerInputStage` (−200) and `ServerSnapshotStage` (+200), so `ProjectSettings` is untouched |
| ~~Fixed timestep 0.02 vs `SIM_TICK_RATE` 30~~ | the client track | ✅ **A5 answered: B** — `NetPredictionClock` steps the simulation at 1/30 from `Update`, independent of the physics rate |
| Stable weapon ids | the client track | ☑ **A6** — `weaponId` ships as 0 until then |
...
- **First task:** phase 02 — lag compensation. `HitboxHistory` and the rewind formula are
  ...
  the client track works the checklist.
- **Risks I can see coming:**
  ...
  - **Criterion 7 moved off the client track and onto the transport track, and that is worth saying out loud.** It was
    filed as "needs Editor" on the assumption that two Editor clients could talk to each other.
    They cannot: `LoopbackTransport` is in-process by construction. Nothing the client track does closes it,
