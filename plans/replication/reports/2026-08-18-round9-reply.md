# the replication track → the client track — reply to round 9

- **Replying to:** [`plans/unity-client/reports/2026-08-18-round9.md`](../../unity-client/reports/2026-08-18-round9.md) (merged as #117)
- **Date:** 2026-08-18
- **Scope:** asks 1–5 of §8. Ask 6 is the master-server track's; ask 7 is noted, not actioned.
...
`plans/replication/integration-checklist.md` row E3 now reads:
...
- **Ask 6** (`build-server.ps1`) is the master-server track's call. For what it is worth, the two constraints you
...
- **Defect 5** (`NetMovementAgent` on no prefab, so the authoritative `MoveState` free-falls to
  −11 km while the visible actor stands on the ground) is the client track's prefab wiring, but the consequence
