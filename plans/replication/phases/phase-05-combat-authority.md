# the replication track — Phase 05: Server combat authority, and the debt cleared before the Editor phase
...
combat class the replication track shipped in phases 01-02 has no caller.
...
5. The four open client minors (M7-M10) are closed, in the files the client track is about to open.
6. All of it is graded by tests that run in CI without opening Unity.

**Not in this phase:** no Editor session, no Profiler. S5 / S4 / S7 stay with the client track, B7 with
The transport track, master-list confirmation with the master-server track.
...
| D8 | Task 6 (the client track file) is last and independent. Everything before it merges without waiting on the client track's review round. |
...
The replication track-owned files only.
...
(the Profiler run that proves it is the client track's S4, out of scope here).
...
### Task 6 — Bot and world damage through the server (1 day + a the client track review round)
...
`Actor.cs` is the client track's file: PR plus their approval, and the schedule assumes one review round.
...
In the files the client track is about to open, so they land before the Editor phase, not during it.
...
| The client track's review round slips the phase | 4 | 2 | 8 | D8 — Task 6 is severable. The reported bug closes at Task 3. |
...
| `WeaponConfig.Rifle` placeholder numbers diverge from the client track's real weapon assets | 3 | 2 | 6 | Documented in the brainstorm report; the seam takes a `WeaponConfig`, so swapping the numbers is data, not code. |
| D9 collapses a serialized field, so any scene or prefab that had authored a `NetServerActor` health value loses it | 3 | 3 | 9 | The value was never read by anything — nothing writes `_health` today, so no authored value is live. Confirm by inspecting the Dustbowl `NetServer` prefabs before removing the field, and note the removal in the Task 6 PR so the client track sees it. |
...
To **The client track**: one PR against `Assembly-CSharp/Actor.cs` (Task 6), one method, with the offline
no-op test attached. Nothing else in this phase touches a the client track file.
...
Still outside the replication track: **B7** (a player id on `ConnectionInfo`, the transport track) and confirming the server
appears in the master's list (the master-server track).
