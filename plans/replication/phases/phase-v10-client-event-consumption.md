# the replication track — Phase V10: The client half of combat, which was never built
...
> `byte[]`) and § 7 (ownership). **Per design § 7, the replication track writes every file here including those under
> `Assembly-CSharp/`**; the client track owns only work that genuinely requires the Editor, enumerated as
> **E1-E12** in § 7.
>
...
| **D5** | **Death drives the ragdoll through the existing public pair, and the ragdoll force is applied to the main rigidbody only.** `Actor.cs:632 public void KnockOver(Vector3 force)` already does `if (!ragdoll.IsRagdoll()) { FallOver(); ApplyRigidbodyForce(force); }` — enable plus force in one public call. **`ApplyRigidbodyForce` (`:641-644`) is hardcoded to `MainRigidbody()` = `rigidbodies[0]`, `ForceMode.Impulse` (`ActiveRaggy.cs:305-308`); there is no per-bone API.** V10 **does not add one**: the bone map would depend on rig bone naming that is authored in the Editor and unreadable from source (already the blocking **E1**), and the phase already carries a the client track PR. |
...
| File | Change |
|---|---|
| `Net/Client/RemoteActorRegistry.cs` | **Edit**, the replication track. Add `public bool TryFind(ushort actorId, out Transform t)` — a `_live.TryGetValue` pass-through. **Named `TryFind` for symmetry with `ServerActorRegistry.cs:109`**, so both sides of the wire read alike. |
| `Net/Client/NetClientPresenterGuard.cs` | **New**, the replication track. `static bool IsPresentable` (`NetContext.IsClient`), **`static bool IsLocalActor(ushort actorId)`** (D2 — the identity predicate that replaces `!aiControlled` everywhere V10 touches), `static bool TryResolveLocalActorId(out ushort id)`, and `static bool TryResolveLocalTeam(out byte team)` (D17). |
...
| File | Change |
|---|---|
| `Net/Client/RemoteActorView.cs` | **New**, the replication track. A component on the pooled remote prefab. `Apply(in ActorSnapshotEntry entry)` consumes the fields § 1.2 shows are decoded and discarded. Exposes `Transform MuzzleAnchor`, `Weapon ActiveWeapon`, `Rigidbody MainRagdollBody`, `byte WeaponId`, `byte Team`, `bool IsLocal`. |
| `Net/Client/RemoteActorRegistry.cs` | **Edit**, the replication track. Resolve the `RemoteActorView` **once on spawn** (`:121-123`) into the pooled entry, never per snapshot — `GetComponent` at 30 Hz × 48 actors is the allocation-free-but-slow trap. Feed `view.Apply(entry)` from the existing interpolation loop. |
...
at all. If any is private, widening it is one additive accessor in the same the client track PR as Task 5.
...
`Assembly-CSharp/MountedTurret.cs` (the client track files — one PR, one review round, per design § 7).
...
**Files:** new `Net/Client/NetClientCombatPresenter.cs` (the replication track). **Needs Tasks 1, 2, 4.**
...
`Net/Client/CosmeticTracerPool.cs` (the replication track); `Assembly-CSharp/Weapon.cs`,
`Assembly-CSharp/IngameUi.cs` (the client track files — same PR as Task 3).
...
**Files:** new `Net/Client/NetClientObjectivePresenter.cs` (the replication track); `Assembly-CSharp/ScoreUi.cs`
(the client track file — same PR as Task 3).
...
`Assembly-CSharp/DecalManager.cs` (the client track files — same PR as Task 3).
...
**Files:** new `Net/Client/NetClientExplosionPresenter.cs` (the replication track). **Supersedes V1 Task 4 (D14).**
...
**Files:** new `tools/ClientWiringGate/` (the replication track), on the `UnitySyntaxCheck` pattern —
...
| The client track PR is now five files (`Actor.cs`, `TankTurret.cs`, `MountedTurret.cs`, `Weapon.cs`, `IngameUi.cs`, `MinimapUi.cs`, `CapturePoint.cs`, `DecalManager.cs`, `ScoreUi.cs`) and conflicts with the client track's branch | 4 | 3 | **12** | One PR, early, announced, with the offline-unchanged tests attached. **Every change is additive or a deleted empty catch** — no signature is removed and no existing caller is broken, so a conflicting merge degrades to a textual conflict rather than a behavioural one. Same precedent as phase-05 Task 6, V1 Task 3 and V8 Task 1. |
...
| 3 — Local-only singleton gating (A16) | **M (2d)** | Needs 1. **Ships with Task 2, not after it.** the client track files. |
...
| 6 — Shooting feedback + tracer + `Hit(int)` | **L (2.5d)** | Needs 1, 2, 4. Grew for D10's from-scratch tracer. The client track files. |
| 7 — Match HUD + `ScoreUi` setter | M (1.5d) | Needs 4 only. **Independent of Task 2.** the client track file. |
| 8 — Capture points | S (1d) | **Blocked on V8 Task 1. Severable, last.** Nothing above waits. |
| 9 — Minimap team fix + empty catch + NPE | S (1d) | Needs 1. **Independent of Task 2.** the client track files. |
...
### To the client track — the Editor half, enumerated

Per design § 7, the client track owns only work that genuinely **requires the Unity Editor**. Every code change
...
**The client track review is one PR** carrying `Actor.cs`, `TankTurret.cs`, `MountedTurret.cs` (Task 3),
...
was overturned** — D10's actual mitigation (open it early, announce it in the PR title before the client track
opens the file) applies unchanged to this phase's the client track PR, which is the second announcement.
