# the replication track — Phase V0: Determinism debt, headless safety, and the seams the vehicle stream needs
...
confirmation is the client track's Editor run — § 7.
...
public  TurretAimLimits aimLimits;          // serialized, per-prefab; the client track fills
...
inside the window, verify hitboxes) is on the client track's Editor list — § 7.
...
The client track's Editor pass (§ 7). Criteria 4, 10 and 13 are fully graded in CI. Design-doc criterion 11
...
|---|---|---|---|---|
| `Car` `Update`→`FixedUpdate` changes single-player handling feel | 5 | 2 | **10** | Accepted and recorded (D1, design-doc § 7 and D8). Task 10 pins the new shape so it does not silently drift back. The client track confirms the feel is playable, not identical — identical is not on offer. |
| Turret rate constants (300 / 600 °/s) are wrong for the prefabs | 4 | 2 | **8** | They are the original per-frame values × 60 fps, i.e. correct at the design framerate, and they are **serialized `aimLimits`** — retuning is a prefab edit by the client track, not a code change. |
| `Actor.cs` conflicts with the client track's branch | 4 | 3 | **12** | D10 — one task, one method, early, announced in the PR title before the client track opens the file. Design-doc § 9 row, same mitigation. |
...
| A source-invariant test passes while the behaviour is still wrong | 3 | 4 | **12** | Stated openly in D7 — they pin shape. Every one of them is paired with a the client track behavioural check in § 7, and criteria 4/10/13 are behavioural in CI. The failure mode this guards against is silent *re*-introduction, which is the one that actually happened. |
...
behaviour is wrong"* — answered by pairing every one with a the client track check — and *"scope creep into
...
- **Task 8 before the client track opens `Actor.cs`** — the only hard ordering constraint against another
  person's work.
...
### Split of work — the replication track writes all code

Per the recorded consumer decision in design-doc § 7, **The replication track writes every line in this phase,
...
phase-05 Task 6: **PR plus a the client track review round**, not silent editing.
...
**No prefab or scene file is edited by the replication track in this phase.** The turret rate constants ship as
code defaults so the phase merges and runs before the client track's prefab pass; the pass tunes them.

### Behavioural checks for the client track's Editor pass
...
### Still outside the replication track

Unchanged from phase-05: **B7** (a player id on `ConnectionInfo`, the transport track) and confirming the server
appears in the master's list (the master-server track). Neither blocks V0.
