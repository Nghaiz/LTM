# the replication track — Phase V1: Explosions, or connecting a wire that has been complete and dead since phase-02
...
> `byte[]`) and § 7 (ownership). Per the design doc § 7, the replication track writes every file in this phase
> including the ones under `Assembly-CSharp/`; the client track owns only the Editor half.
>
...
The replication track-owned Unity files only. Mirrors `ServerTickLoop.EmitDeath` (`:497-522`) and
...
### Task 3 — Authority and suppression inside `ActorManager.Explode` (1 day + a the client track review round)

`Assembly-CSharp/ActorManager.cs` is a the client track file. Per design doc § 7 this is written here, on the
phase-05 Task 6 precedent, with a PR and one the client track review round.
...
> graded by the client track review round and by V9 criterion 11.
...
| `ActorManager.cs` conflicts with the client track's branch | 3 | 3 | 9 | One method, one review round, announced in the PR. Design doc § 9 already sequences V0's `Actor.cs` edit early for the same reason; this rides behind it. |
...
| 3 — `ActorManager.Explode` role split | S (1d) + review round | **Blocked on V0 and on phase-05 Task 6.** The only the client track review in this phase. |
...
**To the client track** — three things, all Editor-only:
